...
void check_fidelity_capaticy(Solution& sol, int k)
{
    ...
    for (int j = 0; j < loop; j++)
    {
        ...
        if( (fidelity >= f_th[i]) && (sol.Rin[i] != 0) && sol.Rin[i] <= R_user_max[i]) // accept --> later have to 1) remove from unserved list, 2) subtract rate, 3) update match
        {
            // cout << fidelity << " " << f_th[i] << " ";
            cout << "ACCEPT\n\n";
            user_served_num++;
        }
        else // refuse --> remove from match --> set -1
        {
            ...
            if(sol.Rin[i] > R_user_max[i])
                cout << "REJECT: EXCEED MAX RATE\n";
            if(sol.Rin[i] == 0)
                cout << "REJECT: NO RATE\n";
            sol.match[i] = -1;
            sol.Rin[i] = 0;
    ...
    // cout << "USER LEFT:" << user_left_num << "\n";
    // check capaccity
    cout << "num of users served by this RIS:" << user_served_num << "\n";
    if(user_served_num > m_k) // exceed capacity --> remove users that performs poorly
    {
        ...
        for(int j=0; j<user_left_num; j++)
        {
            ...
            if(sol.match[i] == k)
            {
                // cout << "user_" << i << " is served by " << sol.match[i] << "\n";
                // int i = sol.user_left[j];
                User input;
                input.id = i;
        ...
        cout << "ranking:\n";
        for(int i=0; i<user_served_num; i++)
        {
            // int i = sol.user_left[j];
...
void SA()
{
    ...
    for(int k=0; k<K; k++) // k: index for RIS
    {
        ...
        check_fidelity_capaticy(sol_current, ris_table[k]);
    ...
    while(T > Tmin)
    {
        for(int l=0; l<L; l++)
        {
            ...
            for(int k=0; k<K; k++) // k: index for RIS
            {
                ...
                check_fidelity_capaticy(sol_new, ris_table[k]);
            }
